...
- **稠密向量记忆**：接 SiliconFlow embedding + sqlite-vec，同义表达也能召回（如"下雨天好美"命中"喜欢下雨天"）；启动时自动为存量记忆补建索引
...
### 主动消息
- **久别主动找**：几小时不说话，菟菚会主动发消息（只对 `PROACTIVE_USER_ID` 指定号，**支持多个**，逗号分隔）
...
- **SQLite WAL 模式**：主连接和向量连接均启用 WAL + busy_timeout，并发读写不锁库
- **发送重试**：主动消息、回复、表情包发送失败自动重试（指数退避），网络抖动不丢消息
- **热梗降级**：刷新失败时继续使用旧缓存，不让热梗突然消失
- **图片下载上限**：识图/收藏限制 8-10MB，防超大文件拖垮内存
