...
- **记忆**：短期保留最近 30 轮；长期记忆 = 原文关键词检索 + LLM 事实提炼（自动记住你的喜好/约定，纯 SQLite）
- **时间感知**：贴合当前现实时间说话（下午/深夜/周末等），开场与发言自然带出
- **联网搜索**：命中新闻/天气/价格等关键词自动联网检索后回答；也可手动 `/搜索 <关键词>`（默认 DuckDuckGo，无需 API key，可用 `SEARCH_ENABLED=0` 关闭）
- **阶段行为**：初识(0-24) → 熟悉(25-49) → 亲密(50-74) → 恋人(75-100)，影响对话尺度

## 已知简化（后续可升级）

- 长期记忆：原文片段用关键词检索 + LLM 定期提炼事实（facts 表，带去重）；纯 SQLite 内置，无外部依赖。如需语义检索，再接入 embedding 向量库
