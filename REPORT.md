...
11. **可观测性**：loguru 文件日志（`data/bot.log`，轮转 10MB），后台任务（每日总结/事实提炼）
...
QQ 小号 ⇄ NapCat(注入 QQNT) ⇄ OneBot WS :3001 ⇄ NoneBot2(bot.py)
                                                     ...
                                                     └─ core/                对话流水线
                                                          ...
                                                          ├─ daily.py     每日提炼(事实+风格)
                                                          ├─ memory.py    短期+长期检索(语义扩展)
                                                          ├─ search.py    联网搜索(博查/Bing/ddg)
                                                          ├─ vision.py    识图(DeepSeek多模态)
                                                          ├─ proactive.py 主动发消息(时段/话题/阶段频率)
...
- **长期记忆**：事实/风格提炼（LLM，约每 10 条消息 + 每日），纯 SQLite；
  语义检索（`MEMORY_SEMANTIC=1`）：疑似回忆时 LLM 查询扩展 + 多词合并打分；可进一步接向量库
